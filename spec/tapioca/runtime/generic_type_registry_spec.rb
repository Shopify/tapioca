...
describe Tapioca::Runtime::GenericTypeRegistry do
  describe ".generic_type_instance?" do
    it "returns false for instances of non-generic classes" do
      refute(Tapioca::Runtime::GenericTypeRegistry.generic_type_instance?(Object.new))
    end

    it "returns true for instances of generic classes" do
      assert(Tapioca::Runtime::GenericTypeRegistry.generic_type_instance?(SampleGenericClass[Object].new))
    end
  end

  describe ".lookup_type_variables" do
    it "returns nil for non-generic types" do
      assert_nil(Tapioca::Runtime::GenericTypeRegistry.lookup_type_variables(Object))
    end

    it "returns the type variables for generic types" do
      assert_equal(
        [SampleGenericClass::Element],
        Tapioca::Runtime::GenericTypeRegistry.lookup_type_variables(SampleGenericClass),
      )
    end
  end
  ...
  describe ".register_type_variable" do
    it " registers a type variable that can be looked up later" do
      ...
      Tapioca::Runtime::GenericTypeRegistry.register_type_variable(not_actually_generic, fake_type_member1)
      Tapioca::Runtime::GenericTypeRegistry.register_type_variable(not_actually_generic, fake_type_member2)

      type_variables = T.must(Tapioca::Runtime::GenericTypeRegistry.lookup_type_variables(not_actually_generic))
