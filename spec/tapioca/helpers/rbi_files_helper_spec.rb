  ...
  describe "#file_diff" do
    it "returns diff when files differ" do
      Dir.mktmpdir do |dir|
        ...
        assert_includes(result, "--- Current x.rbi")
        assert_includes(result, "+++ Correct x.rbi (After running `bin/tapioca dsl`)")
    ...
    it "returns empty string when file path is missing" do
      Dir.mktmpdir do |dir|
        ...
        _out, err = capture_io do
          assert_equal("", file_diff(FILENAME, a, "/nonexistent/path"))
        end
    ...
    it "returns empty string when diff is unavailable" do
      _out, err = capture_io do
        Open3.stub(:capture3, ->(*_args) { raise Errno::ENOENT, "diff" }) do
          assert_equal("", file_diff(FILENAME, "/a", "/b"))
        end
      end
