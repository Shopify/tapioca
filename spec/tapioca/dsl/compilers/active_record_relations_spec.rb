      ...
      class ActiveRecordRelationsSpec < ::DslSpec
        ...
        describe "Tapioca::Dsl::Compilers::ActiveRecordRelations" do
          ...
          describe "decorate" do
            ...
            it "generates proper generic-typed relation classes and modules" do
              add_ruby_file("schema.rb", <<~RUBY)
                ActiveRecord::Migration.suppress_messages do
                  ActiveRecord::Schema.define do
                    create_table :posts do |t|
              ...
              add_ruby_file("roles.rb", <<~RUBY)
                class Role; end
                class Manager < Role; end
                class Default < Role; end
              RUBY

              add_ruby_file("assignable.rb", <<~RUBY)
                module Assignable
                  extend T::Sig
                  extend T::Helpers
                  extend T::Generic
            
                  RoleClass = type_member { { upper: Role } }
            
                  interface!
            
                  sig { abstract.params(role: RoleClass).void }
                  def assign(role); end
                end
              RUBY

              add_ruby_file("restricted_post.rb", <<~RUBY)
                class RestrictedPost < Post
                  extend T::Sig
                  extend T::Generic
                  
                  RoleClass = type_member { { upper: Manager } }
            
                  sig { override.params(role: RoleClass).void }
                  def assign(role); end
                end
              RUBY
              ...
              assert_equal(expected, rbi_for(:RestrictedPost))
            end
