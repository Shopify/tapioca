...
module Tapioca
  class GemSpec < SpecWithProject
    ...
    describe "cli::gem" do
      ...
      it "generates method RBI for URI constant in ruby-lsp" do
        @project.require_real_gem("ruby-lsp", "0.8.0")
        @project.bundle_install
        ...
        result = @project.tapioca("gem ruby-lsp")

        assert_stdout_includes(result, <<~OUT)
          Compiled ruby-lsp
                create  sorbet/rbi/gems/ruby-lsp@0.8.0.rbi
        OUT

        rbi = @project.read("sorbet/rbi/gems/ruby-lsp@0.8.0.rbi")
        assert_includes(rbi, <<~RBI)
          def to_standardized_path; end
        RBI
      end

      it "generates method RBI for URI constant defined in fake gem" do
        foo = mock_gem("foo", "0.0.1") do
          write("lib/foo/internal.rb", <<~RB)
