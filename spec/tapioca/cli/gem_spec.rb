  ...
  class GemSpec < SpecWithProject
    ...
    describe "cli::gem" do
      ...
      describe "flags" do
        ...
        it "ignores annotations if --no-annotations is supplied" do
          repo = create_repo({
            foo: <<~RBI,
              # typed: true
      ...
      describe "generate" do
        ...
        it "generates correct RBI for a gem that has an annotation" do
          repo = create_repo({
            foo: <<~RBI,
              # typed: true
      ...
      describe "annotations" do
        ...
        it "gets annotations from multiple repos" do
          ...
          assert_project_annotation_equal("sorbet/rbi/gems/bar@0.0.1.rbi", <<~RBI)
            # typed: true
        ...
        it "handles conflicts with annotations from multiple repos" do
          ...
          assert_stderr_includes(result, <<~ERR)
            Can't import annotation for `foo` as it contains conflicts:
                Conflicting definitions for `::AnnotationForFoo#foo(x, y)`
                Conflicting definitions for `::AnnotationForFoo#bar()`
          ERR
      ...
      def tapioca_version
        @tapioca_version ||= @project.tapioca("--version").out[/Tapioca v(.*)/, 1]
      end
    end
  end
end
