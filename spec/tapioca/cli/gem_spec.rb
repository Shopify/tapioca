  ...
  class GemSpec < SpecWithProject
    ...
    describe "cli::gem" do
      ...
      describe "generate" do
        ...
        it "ignores `abort` and `exit` calls inside eager loaded gem files" do
          foo = mock_gem("foo", "0.0.1") do
            write!("lib/foo.rb", <<~RB)
              module Foo
                ...
                def self.ensure_checks
                  exit 1
                end
              end

              Foo.check_environment
              Foo.ensure_checks
            RB
          end
          ...
          assert_project_file_equal("sorbet/rbi/gems/foo@0.0.1.rbi", <<~RBI)
            ...
            module Foo
              class << self
                ...
                def ensure_checks; end
              end
            end
          RBI
        end

        it "ignores `abort` and `exit` calls inside autoloaded files" do
          foo = mock_gem("foo", "0.0.1") do
            write!("lib/foo.rb", <<~RB)
              module Foo
