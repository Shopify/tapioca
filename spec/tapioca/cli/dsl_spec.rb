  ...
  class DslSpec < SpecWithProject
    ...
    describe "cli::dsl" do
      ...
      describe "generate" do
        ...
        it "treats reloaded constants as regular named constants" do
          @project.write!("lib/post.rb", <<~RB)
            require "smart_properties"
      ...
      describe "verify" do
        ...
        it "rejects negative --max-diff-lines values" do
          ["0", "-1"].each do |value|
            result = @project.tapioca("dsl --verify --max-diff-lines=#{value}")

            assert_stderr_includes(
              result,
              "Option '--max-diff-lines' must be a positive number",
            )
            refute_success_status(result)
          end
        end
        ...
        it "advises of removed file(s) and returns exit status 1 when files are excluded" do
          ...
          assert_stderr_equals(<<~ERROR, result)
            ...
            Diff:
            --- Current post.rbi
            +++ Expected post.rbi (After running `bin/tapioca dsl`)
            @@ -1,18 +0,0 @@
            -# typed: true
            -
            -# DO NOT EDIT MANUALLY
        ...
        it "advises of new file(s) and returns exit status 1 with new files" do
          ...
          assert_stderr_equals(<<~ERROR, result)
            ...
            Diff:
            --- Current image.rbi
            +++ Expected image.rbi (After running `bin/tapioca dsl`)
            @@ -0,0 +1,18 @@
            +# typed: true
            +
            +# DO NOT EDIT MANUALLY
        ...
        it "truncates diff output to 250 lines when it exceeds the limit" do
          @project.tapioca("dsl")
          ...
          assert_stderr_includes(result, "Diff truncated to 250 lines.")
          assert_stderr_includes(result, "--- Current post.rbi")

          diff_section = result.err.to_s.partition(<<~MESSAGE).last
            Diff truncated to 250 lines.
            Change the number of displayed lines using the `--max-diff-lines` flag.
          MESSAGE
          diff_output = diff_section
          assert_equal(250, diff_output.lines.count)

          refute_success_status(result)
        end

        it "respects a custom --max-diff-lines value" do
          @project.tapioca("dsl")
          ...
          result = @project.tapioca("dsl --verify --max-diff-lines=5")

          assert_stderr_includes(result, "Diff truncated to 5 lines.")

          diff_section = result.err.to_s.partition(<<~MESSAGE).last
            Diff truncated to 5 lines.
            Change the number of displayed lines using the `--max-diff-lines` flag.
          MESSAGE
          diff_output = diff_section
          assert_equal(5, diff_output.lines.count)
