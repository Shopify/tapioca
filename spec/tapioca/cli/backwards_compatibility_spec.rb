...
module Tapioca
  class BackwardsCompatibilitySpec < SpecWithProject
    ...
    describe "T::Class and T.anything usage" do
      before do
        foo = mock_gem("foo", "0.0.1") do
          write("lib/foo.rb", <<~RB)
            Foo = T.type_alias { T::Class[T.anything] }
          RB
        end
        @project.require_mock_gem(foo)
      end

      it "is serialized correctly with Sorbet < 0.5.10820" do
      ...
      it "is a non-generic type with Sorbet >= 0.5.10820" do
