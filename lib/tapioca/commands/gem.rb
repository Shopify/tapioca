...
module Tapioca
  module Commands
    class Gem < Command
      ...
      sig { params(gem: Gemfile::GemSpec, file: RBI::File).void }
      def merge_with_exported_rbs(gem, file)
        tree = gem.exported_rbs_tree
        return file if tree.empty?

        file.root = RBI::Rewriters::Merge.merge_trees(file.root, tree, keep: RBI::Rewriters::Merge::Keep::LEFT)
      rescue RBI::ParseError => e
        say_error("\n\n  RBI files generated by exported RBS files of `#{gem.name}` contain errors and can't be used:", :yellow)
        say_error("Cause: #{e.message} (#{e.location})")
      end
    end
  end
