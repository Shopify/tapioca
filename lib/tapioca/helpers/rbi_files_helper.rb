...
module Tapioca
  module RBIFilesHelper
    ...
    def shims_or_todos_have_duplicates?(nodes, shim_rbi_dir:, todo_rbi_file:)
      ...
      if shims_or_todos.any? { |node| node.is_a?(RBI::Mixin) || node.is_a?(RBI::RequiresAncestor) }
        # Don't forget this method takes a list of nodes with the same qualified name, so if we find two mixins
        # they _are_ about the same thing, like two `include(A)` or two `requires_ancestor(A)` so this is a duplicate
        return true
      end
