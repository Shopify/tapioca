...
module Tapioca
  module CliHelper
    ...
    def netrc_file(options)
      return if options[:annotation_auth]
      return unless options[:annotation_netrc]

      options[:annotation_netrc_file] || ENV["TAPIOCA_NETRC_FILE"] || File.join(ENV["HOME"].to_s, ".netrc")
    end
  end
end
