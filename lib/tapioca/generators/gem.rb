  ...
  module Generators
    class Gem < Base
      ...
      def compile_gem_rbi(gem)
        ...
        strictness =
        rbi = RBI::FileWithConflicts.new(strictness: @typed_overrides[gem.name] || "true")
        compiler = Compilers::SymbolTableCompiler.new
        compiler.compile(gem, rbi, 0, @doc)
        merge_with_exported_rbi(gem, rbi)

        rbi.comments << RBI::Comment.new(
          rbi_header(
        ...
        if rbi.root.empty?
          rbi.comments << RBI::Comment.new(EMPTY_RBI_COMMENT)
        ...
        create_file(filename, rbi.transformed_string)
