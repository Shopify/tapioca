...
module Tapioca
  class Generator < ::Thor::Shell::Color
    ...
    def rbi_header(command, reason: nil, strictness: nil, content_hash: nil)
      hash_line = <<~HASH.chomp if content_hash
        # content_hash: #{content_hash}
        #
      HASH

      statement = <<~HEAD.chomp
      ...
      sigil = <<~SIGIL.chomp if strictness
        # typed: #{strictness}
      SIGIL

      [hash_line, statement, sigil].compact.join("\n").concat("\n")
    end
    ...
    def compile_dsl_rbi(constant, contents)
      ...
      out << rbi_header(
        ...
        content_hash: Digest::SHA256.base64digest(contents)
      )
      out << contents
