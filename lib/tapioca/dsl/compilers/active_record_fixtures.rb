      ...
      class ActiveRecordFixtures < Compiler
        ...
        def method_names
          @method_names ||= begin
            ...
            method_names.select! { |method_name| should_include_method_name?(method_name) }

            method_names
          end #: Array[String]?
        end

        #: (String method_name) -> top
        def should_include_method_name?(method_name)
          # For the method name to be included, it must either be:
          # - missing from the fixture class mapping, which means we will find it a different way, or
          # - the mapping must not be nil, which means we found it in the fixture files without errors
          !fixture_class_mapping_from_fixture_files.key?(method_name) || fixture_class_mapping_from_fixture_files[method_name]
        end
        ...
        def create_fixture_method(mod, name)
          ...
          mod.create_method(name) do |node|
            ...
            node.add_sig do |sig|
              sig.add_param("fixture_name", RBI::Type.simple("::NilClass"))
              sig.add_param("other_fixtures", RBI::Type.simple("::NilClass"))
              sig.return_type = RBI::Type.generic("T::Array", return_type)
            end

            node.add_sig do |sig|
              sig.add_param("fixture_name", RBI::Type.any(RBI::Type.simple("::String"), RBI::Type.simple("::Symbol")))
              sig.add_param("other_fixtures", RBI::Type.simple("::NilClass"))
            ...
            node.add_sig do |sig|
              sig.add_param("fixture_name", RBI::Type.any(RBI::Type.simple("::String"), RBI::Type.simple("::Symbol")))
              sig.add_param("other_fixtures", RBI::Type.any(RBI::Type.simple("::String"), RBI::Type.simple("::Symbol")))
              sig.return_type = RBI::Type.generic("T::Array", return_type)
            end
          end
        end

        #: (String fixture_name) -> RBI::Type
        ...
        def fixture_class_from_fixture_set(fixture_name)
          ...
          RBI::Type.simple(model_name)
        end

        #: -> Hash[String, RBI::Type]
        def fixture_class_from_active_record_base_class_mapping
          @fixture_class_mapping ||=
            ActiveRecord::Base.descendants.each_with_object({}) do |model_class, mapping|
              ...
              mapping[fixture_name] = RBI::Type.simple(class_name)

              mapping
            end #: Hash[String, RBI::Type]?
        end
            ...
            Array(fixture_paths).each_with_object({}) do |path, mapping|
              Dir["#{path}{.yml,/{**,*}/*.yml}"].select do |file|
                ...
                ActiveRecord::FixtureSet::File.open(file) do |fh|
                  ...
                  mapping[fixture_name] = RBI::Type.simple(fh.model_class)
